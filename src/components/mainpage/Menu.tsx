  ...
  return (
    <div className={`w-full absolute bottom-0 z-0 delay-100 transparent transition-all overflow-hidden ease-in-out left-0 ${isClicked ? 'h-screen' : 'h-0'}`}>
        <div className="bg absolute w-full h-full flex justify-between">
          <div className={`bg-side flex justify-center items-center bottom-0 absolute w-6/12 left-0 transition-all delay-200 bg-fuchsia-900 ${isClicked ? 'h-screen' : 'h-0'} bg-ima`}>
            <img src={jpBg} className='opacity-50 w-4/5' alt="" />
          </div>
          <div className={`bg-side bottom-0 absolute w-6/12 overflow-hidden right-0 transition-all delay-300 bg-black  ${isClicked ? 'h-screen' : 'h-0'}`}>
